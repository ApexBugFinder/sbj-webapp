...
def create_deck():
        print('HELLLOOLOOOOOOO:dd', request.json['game.id'] )
        game_id = request.json['game.id']
        print('GAME ID FROM REQUEST************: ', game_id)
        if not game_id:
        ...
        try:
                ...
                for card in docs:
                        ...
                        print('PPPPPPPPPPPOINT .>>>>>>>>>>>>>>>>>>>>>>>>>>2')
                        rt.append(card.serialize())
                        stmt = sqlalchemy.insert(deck_cards_table).values(
                                deck_id=card.deck_id, card_id=card.id, used=card.used)
...
def get_by_id(id: int):
    ...
    try:
        j = deck_cards_table.join(Card).join(
            Deck, Deck.id == deck_cards_table.c.deck_id)
